...
- **Formula**: (2 - 1) / 1 = **+100%**
- **Justification**: Zelkova reports one violation type (UNSAT = unsafe). MIRA distinguishes **two**: `$V$` (intent broken despite safe denial) and `$N$` (neutral/irrelevant). In the external IP case, `$V$` triggers a UX alert (e.g., "Use VPN"), while `$N$` would not.
